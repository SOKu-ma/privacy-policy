...
`アプリ / App` はプルダウンではなくテキスト項目にする。アプリ内からは基本的に事前入力されるため、フォーム側でアプリ選択肢を保守しない。
...
| アプリ | `reorderSupportFormForLowerCognitiveLoad()` 実行後に再取得 |
| OS | `entry.1090172573` |
...
| App | Re-extract after running `reorderSupportFormForLowerCognitiveLoad()` |
...
`アプリ / App` は旧プルダウンを削除してテキスト項目として作り直すため、旧entry IDは使わない。

- 日本語の旧プルダウン entry ID: `entry.1645608167`
- English の旧 dropdown entry ID: `entry.1176267794`

GAS実行ログの `Prefilled URL for entry ID extraction` に出力されるURLから、以下のマーカーに対応する新しい entry ID を取得する。
...
```text
https://docs.google.com/forms/d/e/1FAIpQLSfXp5cAOJq2tXp3apMVFXlDvcrSBMcM8Q_7Ysnpou1ofqeg5Q/viewform?usp=pp_url&entry.2062129080=日本語&entry.<JA_APP_ENTRY_ID>=サッカーフォーメーションボード&entry.1090172573=iOS&entry.848239561=18.5&entry.897002209=1.0.0&entry.1644795001=100&entry.1982869783=App%20Store
```

English:

```text
https://docs.google.com/forms/d/e/1FAIpQLSfXp5cAOJq2tXp3apMVFXlDvcrSBMcM8Q_7Ysnpou1ofqeg5Q/viewform?usp=pp_url&entry.2062129080=English&entry.<EN_APP_ENTRY_ID>=Soccer%20Lineup%20Board&entry.47567059=iOS&entry.498972797=18.5&entry.1930128288=1.0.0&entry.1853635124=100&entry.312784306=App%20Store
```
...
- `アプリ / App` はアプリ側から事前入力するテキスト項目とし、フォーム側で選択肢管理しない
- `OS`、`OSバージョン / OS version`、`アプリバージョン / App version`、`ビルド番号 / Build number`、`ストア / Store` はアプリ側から事前入力するため、ユーザーに最初に選ばせる位置へ置かない
- フォーム項目を削除して作り直すと `entry.xxxxx` が変わるため、アプリ実装後は削除・再作成を避ける
- 例外として、`アプリ / App` は旧プルダウンからテキスト項目へ移行するため一度だけ作り直す
- 表示文言の変更や選択肢追加は可能
